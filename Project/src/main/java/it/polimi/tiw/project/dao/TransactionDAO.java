			...
			try (ResultSet result = pstatement.executeQuery();) {
				if (!result.isBeforeFirst()) 
					...
					ArrayList<Transaction> transactions= new ArrayList<Transaction>();
					 ...
					 while (result.next()) {
						 transactions.add(new Transaction(result.getInt("sourceAccount"),result.getInt("destinationAccount"),result.getInt("id"),result.getDate("date"),result.getFloat("amount"),result.getString("description")));
					    }
					return transactions;
				}
